...
public class ThirdPersonOrbitCamBasic : MonoBehaviour 
{
	...
	private bool isFocusing;										   // Check to see if camera is supposed to be focusing on a target
	private Transform focusTarget;									   // Focus target is the target that camera should focus to
	...
	void Update()
	{
		...
		// Update orientation when locked on to a target
		if (isFocusing) {
			float playerRotatingSpeed = 360f;
			float cameraRotatingSpeed = 230f;

			Vector3 focusTargetStripY = new Vector3 (focusTarget.position.x, 0, focusTarget.position.z);
			Vector3 playerStripY = new Vector3 (player.position.x, 0, player.position.z);
			Quaternion toRotation = Quaternion.LookRotation(focusTargetStripY - playerStripY);
			player.rotation = Quaternion.RotateTowards (player.rotation, toRotation, playerRotatingSpeed * Time.deltaTime);

			Quaternion newCamRotation = Quaternion.RotateTowards(transform.rotation, toRotation, cameraRotatingSpeed * Time.deltaTime);

			angleH = newCamRotation.eulerAngles.y;
		}
	...
	// Focus make camera look at object
	public bool IsFocusing() 
	{
		return isFocusing;
	}

	public void SetFocus(bool setTo) {
		if (focusTarget != null || !setTo) {
			isFocusing = setTo;
		}
	}
